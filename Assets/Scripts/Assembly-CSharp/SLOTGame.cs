...
public class SLOTGame : Singleton<SLOTGame>
{
	...
	public static event Application.LogCallback logCallbackEvent;
	...
	public static Bounds GetBoundsRecursive(List<Transform> targets)
	{
		bool flag = true;
		Bounds result = new Bounds(Vector3.zero, new Vector3(0f, 0f, 0f));
		if (targets != null)
		{
			foreach (Transform target in targets)
			{
				if (target != null)
				{
					Bounds bounds = new Bounds(target.position, GetScale(target));
					if (flag)
					{
						result = bounds;
						flag = false;
	...
	{
		if (Singleton<SLOTAudioManager>.Instance != null)
		{
			Singleton<SLOTAudioManager>.Instance.OnInstantiate(clone);
		}
	}
	...
	public KFFRequestorController GetKFFRequestorController()
	{
		return KFFRequestorController.GetInstance();
	}
	...
	{
		if (Application.isPlaying && SLOTGame.logCallbackEvent != null)
		{
			SLOTGame.logCallbackEvent(logString, stackTrace, type);
		}
	}
}
