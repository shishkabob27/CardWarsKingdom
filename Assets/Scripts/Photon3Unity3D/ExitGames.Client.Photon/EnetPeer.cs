...
namespace ExitGames.Client.Photon
{
	internal class EnetPeer : PeerBase
	{
		...
		internal override void ReceiveIncomingCommands(byte[] inBuff, int dataLength)
		{
			...
			{
				int offset = 0;
				base.Protocol.Deserialize(out short _, inBuff, ref offset);
				byte b = inBuff[offset++];
				byte b2 = inBuff[offset++];
				base.Protocol.Deserialize(out serverSentTime, inBuff, ref offset);
				base.Protocol.Deserialize(out int value2, inBuff, ref offset);
				if (b == 204)
				{
					base.Protocol.Deserialize(out int value3, inBuff, ref offset);
					bytesIn += 4L;
					offset -= 4;
