...
namespace ExitGames.Client.Photon
{
	public abstract class PeerBase
	{
		...
		public string ClientVersion => $"{Version.clientVersion[0]}.{Version.clientVersion[1]}.{Version.clientVersion[2]}.{Version.clientVersion[3]}";

		internal IProtocol Protocol => protocol;
			...
			if (b != 1)
			{
				try
				{
					if (flag)
					{
						...
						memoryStream = new MemoryStream(inBuff, 0, inBuff.Length, writable: true, publiclyVisible: true);
					}
					else
					{
						memoryStream = new MemoryStream(inBuff, 0, inBuff.Length, writable: true, publiclyVisible: true);
