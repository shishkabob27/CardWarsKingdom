...
namespace com.amazon.device.iap.cpt
{
	public abstract class AmazonIapV2Impl : MonoBehaviour, IAmazonIapV2
	{
		...
		public static void FireEvent(string jsonMessage)
		{
				...
				lock (eventLock)
				{
					try
					{
						foreach (IDelegator item in eventListeners[key])
						{
							if (dictionary2 != null)
							{
								item.ExecuteSuccess(new Dictionary<string, object>(dictionary2));
							}
							else
							{
								item.ExecuteSuccess();
					...
					{
						Dictionary<string, List<IDelegator>> dictionary3 = new Dictionary<string, List<IDelegator>>(eventListeners);
						for (int i = 0; i < dictionary3[key].Count; i++)
						{
							IDelegator delegator = dictionary3[key][i];
							if (dictionary2 != null)
							{
			...
			catch (AmazonException ex2)
			{
				logger.Debug("Event call threw exception: " + ex2.ToString());
			}
		}
